...
main:
    ...
    li $v0, 9
    syscall
    add $s4, $v0, $zero # s1 = base address of allocated heap memory
    add $t1, $v0, $zero
...
input_x:
    li $v0, 5
    syscall
    sw $v0, x # store input integer at the address of x
    lw $s1, x # s1 = x
...
binary_search:
    add $s0, $s5, $s3 # s0 = l+r
    srl $s0, $s0, 1 # s0 = i = floor((l+r)/2)
    sll $s0, $s0, 2 # s0 = 4*i
    add $t5, $s4, $s0 # t5 = base address of array + 4*i
    lw $t6, 0($t5) # t6 = a[i]
    beq $t6, $s1, found
